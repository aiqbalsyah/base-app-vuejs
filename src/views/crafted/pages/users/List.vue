  ...
  <div class="card">
    <div class="card-header">
      ...
      <div class="card-toolbar">
        <router-link class="btn btn-sm btn-primary me-3" :to="{ name: 'users-create' }">
          <span>Create New</span>
        </router-link>
      </div>
    </div>
    <div class="card-body">
      <input v-model="searchQuery" type="text" class="form-control mb-4" placeholder="Search" />
      <Datatable :loading="loading" :header="tableHeader" :data="tableData" :total="countData"
        :current-page="(params.offset / params.limit) + 1" :items-per-page="params.limit"
        :sort-label="coloumDefaultLabel" :sort-order="coloumDefaultSort" @page-change="changePage"
        @on-items-per-page-change="changeRowsPerPageLimit" @on-sort="handleSort">
        <template v-slot:imageUrl="{ row: data }">
          <div class="symbol symbol-circle symbol-50px">
            <img :src="getUploadAssetPath(data.imageUrl)" alt="" />
          </div>
        </template>
        <template v-slot:role="{ row: data }">
          {{ data.role }}
        </template>
        <template v-slot:displayName="{ row: data }">
          {{ `${data.displayName}` }}
        </template>
        ...
        <template v-slot:action="{ row: data }">
          ...
          { label: 'View', type: 'link', target: { name: 'users-view', params: { id: data.id } } },
          { label: 'Edit', type: 'link', target: { name: 'users-edit', params: { id: data.id } } },
