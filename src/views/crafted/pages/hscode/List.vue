<template>
  ...
  <div class="card">
    <div class="card-header">
      <h3 class="card-title">HSCODE List</h3>
      <div class="card-toolbar">
        <router-link
              class="btn btn-sm btn-primary me-3"
              :to="{ name: 'hscodes-create' }"
            >
        ...
        <template v-slot:action="{ row: data  }">
          <div class="text-end">
            <DefaultAction :action-list="[
                { label: 'View', type: 'link', target: { name: 'hscodes-view', params: { id: data.id } } },
                { label: 'Edit', type: 'link', target: { name: 'hscodes-edit', params: { id: data.id } } },
...
const deleteData = async (id) => {
  try {
    ...
    await ApiService.delete(`hscodes/${id}`);
    return true;
  } catch (error) {
