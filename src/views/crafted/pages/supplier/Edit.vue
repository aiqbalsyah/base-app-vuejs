              ...
              <label class="col-lg-1 col-md-4 col-form-label fw-semibold fs-6"
                ...
                <div class="col-lg-4 col-md-6">
                  ...
                  <div class="image-input " data-kt-image-input="true"  >
                      ...
                      <div class="image-input-wrapper w-125px h-125px" id="image-input-wrapper"></div>
...
const getData = async (id) => {
  try {
    ...
    const response = await ApiService.query(`suppliers/${id}`, {});
...
const postEditHandle = async (id, params) => {
  ApiService.setHeader();
  return ApiService.patch(`suppliers/${id}`, params)
  .then(({ data }) => {
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchUserData = async () => {
      ...
      countryOptions.value.value = data.country.id;
      originAreaOptions.value.value = data.originArea.id;
      initOriginAreaOptions(data.country.id,data.originArea.id);
    };
    ...
    const initOriginAreaOptions = async (countryId:string,defaultVal:string | null) => {
        await HelperService.getOriginAreas({
          ...
          originAreaOptions.value.value = defaultVal;
    ...
    const handleOnSelectCountry = async (value: string, option: object) => {
      ...
      initOriginAreaOptions(countryId,null);
    }
    ...
    onMounted( async () => {
      ...
      setTimeout(() => {
          ...
          if (imageWrapper) {
            ...
            console.log(record.value.imageUrl)
            imageWrapper.style.backgroundImage = `url("${getUploadAssetPath(record.value.imageUrl)}")`;
          }
      }, 1000);
    });
